...
def run_with_flags(generator):
  ...
  for i in range(FLAGS.num_outputs):
    generator_options.args['temperature'].float_value = random.uniform(0.5, 0.95)
    print("temperature: ", generator_options.args['temperature'].float_value )
    generated_sequence = generator.generate(input_sequence, generator_options)

    midi_filename =""
    if FLAGS.is_csv:
        midi_filename=str(i+1)+FLAGS.file_name+".mid"
    else:
        midi_filename = FLAGS.file_name+str(i+1)+".mid"
